...
{
    Mat srcImage = imread("Gamma.jpg");
    if (!srcImage.data)
    {
        cerr << "fail to load image" << endl;
        exit(-1);
    }
    Mat dstImage, dstImage1,dstImage_gray;
    cv::cvtColor(srcImage, dstImage_gray, cv::COLOR_BGR2GRAY);
    GammaTransform(dstImage_gray, dstImage, 0.5);
    GammaTransform(dstImage_gray, dstImage1, 2.0);
    imshow("ԭͼ", dstImage_gray);
    imshow("gamma 0.5", dstImage);
    imshow("gamma 2.0", dstImage1);
    waitKey(0);
    destroyAllWindows();
    return 0;
}
